...
that the go tool will use the copied packges when run
...
To try this tool on a project that uses godep save -r,
first save the project without -r (to undo rewriting of the
import paths), then run this tool as you normally would:

	cd my/project
	GOPATH=/tmp/move godep restore
	GOPATH=/tmp/move godep save ./... # no -r
	vexp -v
